...
public class Tabuleiro {
	...
	public static int analisaMovimento(String coordi, String coordf) {
		...
		int condicaoDaJogada = verificaJogadaSuicida(coordi, coordf);
		
		if (condicaoDaJogada == 0) // jogada pode ser feita
			return verificaXeque(coordPecaDaVez, coordReiAdversario);
		return condicaoDaJogada;
	}
	...
	public static int verificaXeque(String coordPecaXeque, String coordRei) {
		Peca pecaRei = Tabuleiro.getCasa(coordRei);
		if (pecaRei != null) {
			int jogadorRei = pecaRei.getCor();
			...
			for (int i = 0; i < 63; i++) {
				if (Tabuleiro.casas[i] != null)
					if ((Tabuleiro.casas[i]).getCor() != jogadorRei) {
						...
						if ((Tabuleiro.casas[i]).jogadaValida(posrX, posrY)) {
							if (Tabuleiro.xequeMate((Tabuleiro.casas[i]).getPosicao(), coordRei)) {
	...
	public static boolean xequeMate(String coordPecaXeque, String coordRei) {
		...
		// peca que realiza o xeque

		for (int i = 0; i < 63; i++) {
			Peca pecaDeTeste = Tabuleiro.casas[i];
			if (pecaDeTeste != null) {

				// verifica se alguma peca pode comer a que aponta para o rei //
				String coordPecaSalvadora = pecaDeTeste.getPosicao();
				
				if (pecaDeTeste.getCor() != pecaXeque.getCor()
						&& Tabuleiro.verificaJogadaSuicida(pecaDeTeste.getPosicao(),
										coordPecaXeque) == 0) {
				...
				return !verificaSeReiPodeFugir(pecaXeque, coordRei);
	...
	private static boolean verificaSeReiPodeFugir(Peca pecaXeque, String coordRei) {

		char posRY = Tabuleiro.separaCoordenadaY(coordRei);
		int posRX = Tabuleiro.separaCoordenadaX(coordRei);
		
		boolean movimentoFugaValido = false;
		
		movimentoFugaValido = movimentoFugaValido(pecaXeque, coordRei, posRX, (char)(posRY + 1)) || movimentoFugaValido;
				
		movimentoFugaValido = movimentoFugaValido(pecaXeque, coordRei, posRX + 1, (char)(posRY + 1)) || movimentoFugaValido;
		
		movimentoFugaValido = movimentoFugaValido(pecaXeque, coordRei, posRX + 1, posRY) || movimentoFugaValido;
		...
		movimentoFugaValido = movimentoFugaValido(pecaXeque, coordRei, posRX - 1, (char)(posRY)) || movimentoFugaValido;

		movimentoFugaValido = movimentoFugaValido(pecaXeque, coordRei, posRX - 1, (char)(posRY + 1)) || movimentoFugaValido;
		
		return movimentoFugaValido;
	}

	private static boolean movimentoFugaValido(Peca pecaXeque, String coordRei, int coordRX, char coordRY) {
		int corDoDei = Tabuleiro.getCasa(coordRei).getCor();
		
		if ( coordRY >= 'a' && coordRY <= 'h' && coordRX >= 1 && coordRX <= 8 ) {
			
			String coordDest = coordRY + "" + coordRX;
			Peca casaDestino = Tabuleiro.getCasa(coordDest);
			
			if ((casaDestino != null && casaDestino.getCor() != corDoDei)
					&& pecaXeque.jogadaValida(coordRX, coordRY)
					&& Tabuleiro.verificaJogadaSuicida(coordRei, coordDest) == 0 ){
	...
	private static boolean pecaIntermXeque(Peca pecaXeque) {
		...
		for (int i = 0; i < 63; i++) {
			Peca pecaTest = Tabuleiro.casas[i];
			
			if (pecaTest != null) {
				
				// Peca pecaCasa = Tabuleiro.casas[i];
				if (pecaTest.getCor() != pecaXeque.getCor()
						&& pecaTest.jogadaValida(posPecaXequeX, posPecaXequeY)) {
